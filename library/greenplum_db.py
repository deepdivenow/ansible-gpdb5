...
def get_db_info(cursor, db):
    ...
    pg_database.datconnlimit AS conn_limit
...
def db_create(cursor, db, owner, template, encoding, conn_limit):
    params = dict(enc=encoding, conn_limit=conn_limit)
...
def db_matches(cursor, db, owner, template, encoding, conn_limit):
    if not db_exists(cursor, db):
        return False
    else:
        db_info = get_db_info(cursor, db)
    ...
    try:
        if module.check_mode:
            if state == "absent":
                ...
                changed = not db_matches(cursor, db, owner, template, encoding, conn_limit)
        ...
        if state == "absent":
            ...
            try:
                changed = db_create(cursor, db, owner, template, encoding, conn_limit)
            except SQLParseError as e:
                module.fail_json(msg=to_native(e), exception=traceback.format_exc())
